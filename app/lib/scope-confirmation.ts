...
export function createScopeConfirmationToken(payload:Omit<ScopeConfirmationPayload,"exp">,secret:string){
  const fullPayload:ScopeConfirmationPayload={...payload,exp:Date.now()+7*24*60*60*1000};
...
export function verifyScopeConfirmationToken(token:string,secret:string){
  ...
  try{const payload=JSON.parse(Buffer.from(encoded,"base64url").toString("utf8")) as ScopeConfirmationPayload;if(!payload.exp||payload.exp<Date.now())return null;return payload;}catch{return null;}
}
